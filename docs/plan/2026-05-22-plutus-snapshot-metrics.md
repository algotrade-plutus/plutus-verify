# Plutus v2 Spec — `plutus snapshot --metrics` (Plan 8)

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development.

**Goal:** Extend `plutus snapshot` to fill `expected.metrics[].value` in the manifest from the metrics each step produced in `.plutus/run/<step_id>/results.json`. The author runs snapshot, reviews the diff, and `git commit`s — the commit is the verification claim.

**Architecture:** Use `ruamel.yaml` for round-trip-preserving YAML edits (preserves the author's comments and formatting). Extend `scaffold_snapshot` to do reference-output copying *and* metric-value filling in the same pass. CLI defaults to both; flags let authors opt out of either side.
...
4. ❌ **Toil:** for each metric they want to verify, they hand-type the value from the script's output into the manifest YAML
...
2. **One default, two opt-outs.** `plutus snapshot` defaults to filling both reference outputs AND metric values. Two CLI flags: `--no-reference-outputs` and `--no-metrics` for authors who want only one side.
3. **No auto-creation of metrics.** If results.json has a metric named `sortino_ratio` but the manifest has no `sortino_ratio` metric declared, snapshot **skips** with a warning. Adding new metrics would be magic — author controls manifest structure.
...
5. **Skip missing results.json gracefully.** If a step's results.json doesn't exist (script didn't write one, step was skipped, etc.), snapshot reports it in `notes` and leaves the manifest's metric values alone.
6. **Refuse to snapshot on hard-failed checks.** Existing snapshot refuses on `check_result.exit_code == 2` (a required step failed). Keep that. ExpectedMetric divergence (exit 1) is fine — that's exactly when the author wants to update the values.
...
- `plutus_verify/scaffold/snapshot.py` — extend `scaffold_snapshot` to also update metric values
- `plutus_verify/__main__.py` — CLI `snapshot` subcommand gains `--no-reference-outputs` and `--no-metrics` flags
- `pyproject.toml` — add `ruamel.yaml>=0.18` to core dependencies

**New:**

- `plutus_verify/scaffold/manifest_edit.py` — `update_metric_values(manifest_path, updates)` function. Uses ruamel.yaml. Pure function (no I/O outside the manifest file).

**Tests:**

- `tests/unit/test_manifest_edit.py` — round-trip tests, comment preservation, value overwrite, missing-block handling
- `tests/unit/test_scaffold_snapshot.py` (extend) — snapshot now updates metric values; refuses on exit 2; respects flags
...
def update_metric_values(
    ...
    """Update `expected.metrics[].value` in-place in the manifest YAML.

    Args:
        manifest_path: Path to `.plutus/manifest.yaml`.
        updates: {step_id: {metric_name: new_value}}. Values are written
    ...
    Returns:
        ...
        metric_name present in updates but no declared metric.

    Behavior:
        - Comments, blank lines, and key order are preserved (ruamel.yaml
          round-trip mode).
        - If `updates[step_id]` is empty, the step is ignored.
        - If `updates[metric_name]` doesn't match an existing metric's
...
def scaffold_snapshot(
    ...
    *,
    ...
    update_metric_values: bool = True,      # NEW
) -> SnapshotResult: ...
...
## Task 1: `manifest_edit.update_metric_values`
...
- [ ] **Step 3 — failing test for round-trip preservation**: write a small manifest YAML with comments and blank lines; call `update_metric_values` to change ONE value; assert the output:
...
- [ ] **Step 5 — failing test for missing metric warning**: updates has `metric_name="bogus"` but manifest's expected block doesn't declare it; assert warning text mentions the name; manifest unchanged for that update.
...
- [ ] **Step 8 — implement `update_metric_values`** with ruamel.yaml round-trip. Use `YAML(typ='rt')`. Mutate values in place. Re-emit to the file.
- [ ] **Step 9 — run tests**: `source .venv/bin/activate && pytest tests/unit/test_manifest_edit.py -v`
- [ ] **Step 10 — commit**.

Commit:
```
feat(scaffold): manifest_edit.update_metric_values via ruamel.yaml

Adds round-trip-preserving manifest editor that overwrites
expected.metrics[].value entries by name. Preserves comments, blank
lines, key order. Warns (does not error) on unknown step_ids and
metric names — those become snapshot notes.
...
if update_metric_values:
    ...
    for er in manifest.expected:
        ...
        try:
            ...
            notes.append(f"step '{er.step_id}': no results.json — metrics not updated")
            continue
        except MalformedResultsError as exc:
            notes.append(f"step '{er.step_id}': malformed results.json — {exc}")
            continue
        # Only update values for metrics that the manifest declares
    ...
    if updates:
        ...
        try:
            count, warnings = update_metric_values_in_yaml(manifest_path, updates)
...
(Rename the imported function to `update_metric_values_in_yaml` to avoid clashing with the kwarg.)

TDD sub-steps:
- [ ] **Step 1 — failing test for happy path**: pre-populate `.plutus/run/in_sample/results.json` via the SDK; call `scaffold_snapshot(run_check_first=False)`; assert `metrics_updated > 0`; read the manifest and verify the value matches the results.json metric.
- [ ] **Step 2 — failing test for `update_metric_values=False`**: same setup; assert `metrics_updated == 0`; manifest unchanged.
- [ ] **Step 3 — failing test for missing results.json**: manifest declares metrics, no results.json written; assert `metrics_updated == 0`; notes contain "no results.json — metrics not updated".
- [ ] **Step 4 — implement**
...
Commit:
```
feat(scaffold): plutus snapshot fills metric values from results.json

scaffold_snapshot now reads .plutus/run/<step_id>/results.json for every
step that has declared metrics and overwrites the matching
expected.metrics[].value entries in the manifest. Toggleable via
update_metric_values kwarg (default True). Refuses to update from
...
@click.option("--no-metrics", is_flag=True, help="...")
# ... other existing options ...
def snapshot_cmd(repo_path, no_reference_outputs, no_metrics, ...):
    ...
    ...
    click.echo(f"  metrics updated: {result.metrics_updated}")
    if result.notes:
        click.echo("notes:")
...
TDD sub-steps:
- [ ] **Step 1 — failing test for `--no-metrics`**: invoke CLI with the flag; assert `update_metric_values=False` was passed to scaffold_snapshot (via mock or by checking the output).
...
Commit:
```
feat(cli): plutus snapshot --no-metrics / --no-reference-outputs
...
fills metric values from results.json by default. The two opt-out
...
**Files:**
- Modify in-place: `out/transfer-test/ProtoMarketMaker/.plutus/manifest.yaml` (set a few metric values to deliberately wrong numbers; gitignored, so no commit)
...
- [ ] **Step 2 — run snapshot**:
  ...
  ```
  This runs `plutus check` first (~6-10 min) then snapshots. Expected: `metrics updated: 12` (6 in-sample + 6 OOS).
...
- [ ] **Step 4 — re-run `plutus check`** against the now-snapshotted manifest. Same metric pass pattern as before (6/6 in-sample, 3/6 OOS) — the OOS divergence comes back because the OOS metrics snapshot now matches the SCRIPT's actual values (not the README's claimed values).
...
Commit:
```
docs: Plan 8 complete — plutus snapshot now writes metric values

plutus snapshot now extracts values from .plutus/run/<step_id>/results.json
and writes them into expected.metrics[].value via ruamel.yaml
...
- **Auto-creating metrics** for metrics in results.json that aren't declared in the manifest. Author controls structure.
