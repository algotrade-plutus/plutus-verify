...
| **Standardized / template** | 10% | **10** = canonical 4-step shape (`data_collection → in_sample_backtest → optimization → out_of_sample_backtest`), externalized `parameter/*.json`, predictable `result/{backtest,optimization}/` paths, no module-level side effects; **5** = one significant deviation; **0** = needs rework. |
