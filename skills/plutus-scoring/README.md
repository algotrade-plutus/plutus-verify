...
Invokable standalone on any v2-compliant repo, or chained automatically from [`plutus-transform`](../plutus-transform/) as its final hand-off step.
...
If the target repo isn't v2-compliant yet (no `.plutus/manifest.yaml`), the Skill fails fast and points the operator at `plutus-transform` or `plutus init`.
...
## Relationship to plutus-transform

| | plutus-transform | plutus-scoring |
|---|---|---|
...
| **Invocation** | Manual (user-triggered) | Standalone OR auto-chained from plutus-transform |
