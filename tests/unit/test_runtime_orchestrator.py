...
def test_runtime_runs_all_steps_and_compares_headlines(tmp_path):
    ...
    # Headline comparison itself is wired in Task 4 (results.json reader). For
    # now we only verify the orchestrator still returns a placeholder entry.
    assert "sharpe" in result.headline_results["in_sample"]
...
# Plan 6 / Task 3 removed the locator vocabulary from v2 headlines: the SDK
# now writes a canonical .plutus/run/<step_id>/results.json and the orchestrator
# will read it natively in Task 4. The previous tests in this slot exercised
# locate.kind=stdout_table dispatch, which no longer exists — they will be
# replaced in Task 4 with results.json-based assertions.
